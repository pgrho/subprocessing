...
namespace Shipwreck.Subprocessing
{
    ...
    public abstract class DebuggerInfoProvider
    {
        ...
        /// </returns>
        public static DebuggerInfo GetCurrent()
        {
            lock (_Providers)
            {
                foreach (var p in _Providers)
                {
                    var i = p.GetCurrentDebuggerInfo();
                    if (i != null)
                    {
                        return i;
                    }
        ...
        public static bool AttachTo(DebuggerInfo debuggerInfo)
        {
            ...
            lock (_Providers)
            {
                foreach (var p in _Providers)
                {
                    if (p.AttachToType(debuggerInfo))
                    {
                        return true;
                    }
                }
